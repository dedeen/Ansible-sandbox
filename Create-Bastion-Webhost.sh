#### This script will build an EC2 bastion host in a preexisting subnet in the webserver VPC.
...
which_bastion_host=1          # 1: only choice currently
...
if [ $which_bastion_host -eq 1 ]
   then 
      ...
      bastion_subnet=websrv-az1-inst
      bh_igw_name=web-bastion1-IGW
      bh_rt_name=web-Bastion-Host1-RT
      bh_ec2_name=web-Bastion-Host1
      bh_vpc_name=Usr01-VPC-intra         # Actually name of default RT built by terraform
      bh_vpc=WebSrv-VPC                    # Name of the VPC that the bastion host will be created in
  fi
